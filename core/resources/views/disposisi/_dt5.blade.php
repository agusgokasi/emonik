<!-- .modal -->
    <div id="m-dt5{{$permohonan->slug}}" class="modal fade" data-backdrop="true">
        <div class="modal-dialog" id="animate">
            ...
            <form method="post" role="form" action="{{ route('dis5Tolak' , ['permohonan' => $permohonan->slug]) }}" enctype="multipart/form-data" id="myForm">
                ...
                <div class="modal-body text-left p-lg">
                    ...
                    <div class="form-group row">
                        <label for="spj_tolak_kas" class="col-md-4 col-form-label text-md-left">{{ __('Bukti Penolakan') }}</label>
                        <div class="col-md-8">
                            <input type="file" name="spj_tolak_kas" class="form-control{{ $errors->has('spj_tolak_kas') ? ' is-invalid' : '' }}" id="spj_tolak_kas" value="{{ old('spj_tolak_kas') }}" required>
                            @if ($errors->has('spj_tolak_kas'))
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $errors->first('spj_tolak_kas') }}</strong>
                                </span>
                            @endif
                        </div>
                    </div>
